...
socket.on('question-error', ({ question, error, rawResponse, aiResponse }) => {
  console.error('Question processing error:', { question, error });
  console.error('Raw Response:', rawResponse);
  console.error('Full AI Response:', aiResponse);
  addQuestionResult(question, 'Invalid Question Provided', [], rawResponse, aiResponse);
});
...
function addQuestionResult(question, answer, pageNumbers, rawResponse, aiData) {
  console.log('Adding question result:', { question, answer, pageNumbers, rawResponse, aiData });
  const questionItem = document.createElement('div');
  questionItem.className = 'question-item';

  // Parse the AI response to extract answer and reason
  const answerMatch = answer.match(/([A-Z]\. [^\n]+)\n\nReason: (.+)/);
  let formattedAnswer = answer;
  let reason = '';

  if (answerMatch) {
    formattedAnswer = answerMatch[1];
    reason = answerMatch[2];
  }

  questionItem.innerHTML = `
    <div class="question">${question}</div>
    <div class="answer ${answer === 'Invalid Question Provided' ? 'error' : 'correct'}">${formattedAnswer}</div>
    ${reason ? `<div class="reason">${reason}</div>` : ''}
    <div class="pages">Relevant Pages: ${pageNumbers.join(', ')}</div>
    ${aiData ? `<div class="raw-response"><pre>${JSON.stringify(aiData, null, 2)}</pre></div>` : ''}
  `;
